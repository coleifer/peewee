...
.. tip::
   ...
   schema from scratch, the :ref:`pwiz` tool can generate model definitions
   automatically by introspecting the database.
...
Peewee creates a separate table for each concrete model class. There is no
notion of abstract base classes at the database level - if you subclass a model,
both the parent and the child get their own tables.
...
.. code-block:: python
   ...
   # Descending index with expression.
...
foreign key which links a child object to its parent. Because the model class
is not defined at the time you instantiate the self-referential foreign key,
use the special string ``'self'`` to indicate a self-referential foreign key.

The foreign key should be nullable so that root nodes can have no parent:
