...
:class:`SqliteExtDatabase`, that will serialize concurrent writes to a
SQLite database. :class:`SqliteQueueDatabase` can be used as a drop-in
replacement for the regular :class:`SqliteDatabase` if you want simple
...
:class:`SqliteQueueDatabase` is designed to simplify things by sending all
...
interleaved, the :meth:`~SqliteQueueDatabase.transaction` and
:meth:`~SqliteQueueDatabase.atomic` methods are disabled on :class:`SqliteQueueDatabase`.

For cases when you wish to temporarily write to the database from a different
thread, you can use the :meth:`~SqliteQueueDatabase.pause` and
:meth:`~SqliteQueueDatabase.unpause` methods. These methods block the
...
The :meth:`~SqliteQueueDatabase.stop`, :meth:`~SqliteQueueDatabase.start`,
and :meth:`~SqliteQueueDatabase.is_stopped` methods can also be used to
...
:class:`SqliteQueueDatabase` accepts some special parameters which you
...
:meth:`~SqliteQueueDatabase.start` to bring up the worker threads that will
...
database, you will need to call :meth:`~Database.connect` and
:meth:`~Database.close` as you would with any other database instance.

When your application is ready to terminate, use the :meth:`~SqliteQueueDatabase.stop`
...
Lastly, the :meth:`~SqliteQueueDatabase.is_stopped` method can be used to
...
:class:`APSWDatabase` extends the :class:`SqliteExtDatabase` and inherits
...
.. class:: SqlCipherDatabase(database, passphrase, **kwargs)
    ...
    Subclass of :class:`SqliteDatabase` that stores the database
    ...
    around ``sqlite3``, so the API is *identical* to :class:`SqliteDatabase`'s,
...
* :class:`ArrayField` field type, for storing arrays.
* :class:`HStoreField` field type, for storing key/value pairs.
* :class:`IntervalField` field type, for storing ``timedelta`` objects.
* :class:`JSONField` field type, for storing JSON data.
* :class:`BinaryJSONField` field type for the ``jsonb`` JSON data type.
* :class:`TSVectorField` field type, for storing full-text search data.
* :class:`DateTimeTZField` field type, a timezone-aware datetime field.
...
.. warning:: In order to start using the features described below, you will need to use the extension :class:`PostgresqlExtDatabase` class instead of :class:`PostgresqlDatabase`.
...
:class:`JSONField`. As of version 2.4.7, peewee also supports the Postgres
9.4 binary json ``jsonb`` type, via :class:`BinaryJSONField`.
...
.. warning::
  ...
  To use :class:`BinaryJSONField`, which has many performance and querying
  advantages, you must have Postgres 9.4 or later.

.. note::
  You must be sure your database is an instance of
  :class:`PostgresqlExtDatabase` in order to use the `JSONField`.
...
The :class:`BinaryJSONField` works the same and supports the same operations
as the regular :class:`JSONField`, but provides several additional
...
(:meth:`~BinaryJSONField.contains`, :meth:`~BinaryJSONField.contains_any`,
:meth:`~BinaryJSONField.contains_all`), or whether it is a subset of a
larger JSON document (:meth:`~BinaryJSONField.contained_by`).

For more examples, see the :class:`JSONField` and
:class:`BinaryJSONField` API documents below.
...
instantiating your :class:`PostgresqlExtDatabase`:
...
is as simple as adding a :class:`HStoreField` to your model:
...
using the :func:`ServerSide` helper:
...
cursor, you can specify this when creating your :class:`PostgresqlExtDatabase`:
...
.. note::
    ...
    If you are using the :func:`ServerSide` helper, the transaction and
...
For simple cases, you can simply use the :func:`Match` function, which will
...
Alternatively, you can use the :class:`TSVectorField` to maintain a
...
.. note::
    :class:`TSVectorField`, will automatically be created with a GIN index.
...
To perform a full-text search, use :meth:`TSVectorField.match`:
...
.. class:: PostgresqlExtDatabase(database, server_side_cursors=False, register_hstore=False, prefer_psycopg3=False, ...)

    Identical to :class:`PostgresqlDatabase` but required in order to support:
...
.. function:: ServerSide(select_query)
    ...
    :meth:`~SelectQuery.iterator` method to avoid caching row instances.
...
.. class:: ArrayField(field_class=IntegerField, field_kwargs=None, dimensions=1, convert_values=False)
    ...
    .. method:: contains_any(*items)
        ...
        Like :meth:`~ArrayField.contains`, except will match rows where the
...
.. class:: DateTimeTZField(*args, **kwargs)
    ...
    A timezone-aware subclass of :class:`DateTimeField`.
...
.. class:: HStoreField(*args, **kwargs)
    ...
    .. attention::
        To use the :class:`HStoreField` you will need to be sure the
        *hstore* extension is registered with the connection. To accomplish
        this, instantiate the :class:`PostgresqlExtDatabase` with
...
.. class:: JSONField(*args, **kwargs)
    ...
    .. note::
        Strongly consider using the :class:`BinaryJSONField` instead as it
...
.. class:: BinaryJSONField(*args, **kwargs)
    ...
    :class:`JSONField`. The example queries below will use the same
    ...
    .. method:: contains_any(*items)
        ...
        Like :meth:`~BinaryJSONField.contains`, we can also search sub-keys:
    ...
    .. method:: contains_all(*items)
        ...
        Like :meth:`~BinaryJSONField.contains_any`, we can also search sub-keys:
    ...
    .. method:: contained_by(other)
        ...
        Test whether the given JSON document is contained by (is a subset of) the given JSON document. This method is the inverse of :meth:`~BinaryJSONField.contains`.
...
.. class:: MariaDBConnectorDatabase(database, **kwargs)
    ...
    .. note::
        The :class:`MariaDBConnectorDatabase` does **not** accept the following
        parameters:
...
.. class:: JSONField()
    ...
    Extends :class:`TextField` and implements transparent JSON encoding and
    decoding in Python.
...
.. function:: Match(columns, expr, modifier=None)
    ...
    :noindex:
    :param columns: a single :class:`Field` or a tuple of multiple fields.
...
You can export or import data using :meth:`~DataSet.freeze` and
:meth:`~DataSet.thaw`:
...
:class:`DataSet` objects are initialized by passing in a database URL of the
...
We can now :meth:`~Table.insert` new rows into the table. If the columns do
...
the :meth:`~Table.thaw` method. By default, new columns will be created for
...
You can use the :meth:`tables` method to list the tables in the current
database:
...
To retrieve all rows, you can use the :meth:`~Table.all` method:
...
Specific objects can be retrieved using :meth:`~Table.find` and
:meth:`~Table.find_one`.
...
To export data, use the :meth:`~DataSet.freeze` method, passing in the query
you wish to export:
...
.. class:: DataSet(url, **kwargs)
    ...
    :param url: A database URL or a :class:`Database` instance. For
    ...
    .. method:: __getitem__(table_name)
        ...
        Provide a :class:`Table` reference to the specified table. If the
...
.. class:: CompressedField(compression_level=6, algorithm='zlib', **kwargs)
    ...
    :class:`BlobField`, transparently storing a compressed representation of
    the data in the database.

.. class:: PickleField()

    Stores arbitrary Python data by transparently pickling and un-pickling data
    stored in the field. This field extends :class:`BlobField`. If the
...
.. class:: KeyValue(key_field=None, value_field=None, ordered=False, database=None, table_name='keyvalue')
    ...
    :param Field key_field: field to use for key. Defaults to
        :class:`CharField`. **Must have** ``primary_key=True``.
...
.. function:: model_to_dict(model, recurse=True, backrefs=False, only=None, exclude=None, extra_attrs=None, fields_from_query=None, max_depth=None, manytomany=False)
    ...
    :param Select fields_from_query: The :class:`SelectQuery` that created this model instance. Only the fields and values explicitly selected by the query will be serialized.
...
.. class:: ThreadSafeDatabaseMetadata()
    ...
    Model :class:`Metadata` implementation that provides thread-safe access
...
.. warning::
    ...
    use the :meth:`Model.insert`, :meth:`Model.update`, or
    :meth:`Model.delete` methods. These methods generate queries that
    ...
    :meth:`Model.save` and :meth:`Model.delete_instance`, where the
...
.. class:: Signal()
    ...
    .. method __call__(name=None, sender=None)
...
* All the parameters of the :class:`ForeignKeyField` are explicitly
...
:class:`Database` object, it should be easy to manage changing your model
...
Instantiate a ``migrator``. The :class:`SchemaMigrator` class is responsible
for generating schema altering operations, which can then be run sequentially
by the :func:`migrate` helper.
...
Use :func:`migrate` to execute one or more operations:
...
.. warning::
    ...
    :meth:`~Database.atomic` context-manager, e.g.
...
.. note::
    ...
    the column name for a given :class:`ForeignKeyField`. When adding a
...
.. class:: Introspector(metadata, schema=None)
    ...
    :class:`Introspector`. Rather than instantiating this class directly, it
    is recommended to use the factory method
    :meth:`~Introspector.from_database`.
...
.. function:: connect(url, unquote_password=False, unquote_user=False, **connect_params)
    ...
    Create a :class:`Database` instance from the given connection URL.
    ...
    * *mysql+pool://user:passwd@ip:port/my_db?max_connections=20&stale_timeout=300* will create a :class:`PooledMySQLDatabase` instance for the local MySQL database *my_db* with max_connections set to 20 and a stale_timeout setting of 300 seconds.
    ...
    * ``psycopg3``: :class:`PostgresqlDatabase`
    * ``psycopg3+pool``: :class:`PooledPostgresqlDatabase`
    * ``sqlite``: :class:`SqliteDatabase`
    * ``sqliteext``: :class:`SqliteExtDatabase`
    * ``sqlite+pool``: :class:`PooledSqliteDatabase`
    * ``sqliteext+pool``: :class:`PooledSqliteExtDatabase`
...
The ``pool`` module contains a number of :class:`Database` classes that
...
.. class:: PooledDatabase(database, max_connections=20, stale_timeout=None, timeout=None, **kwargs)
    ...
    Mixin class intended to be used with a subclass of :class:`Database`.
...
.. class:: PooledPostgresqlDatabase
    ...
    Subclass of :class:`PostgresqlDatabase` that mixes in the :class:`PooledDatabase` helper.

.. class:: PooledPostgresqlExtDatabase

    Subclass of :class:`PostgresqlExtDatabase` that mixes in the :class:`PooledDatabase` helper. The :class:`PostgresqlExtDatabase` is a part of the
    :ref:`postgres_ext` module and provides support for many Postgres-specific
    features.

.. class:: PooledMySQLDatabase

    Subclass of :class:`MySQLDatabase` that mixes in the :class:`PooledDatabase` helper.
...
.. class:: PooledSqliteExtDatabase
    ...
    Persistent connections for SQLite apps, using the :ref:`sqlite_ext` advanced database driver :class:`SqliteExtDatabase`.

.. class:: PooledCySqliteDatabase

    Persistent connections for SQLite apps, using the :ref:`cysqlite_ext` database driver :class:`CySqliteDatabase`.
...
The :class:`FlaskDB` class is a wrapper for configuring and referencing a
...
:class:`PostgresqlDatabase` specified by the given database URL. Request
...
:class:`FlaskDB` class exposes a :attr:`FlaskDB.Model` property which can
...
peewee :class:`Database` instance:
...
Using a peewee :class:`Database` object:
...
the :class:`FlaskDB` class implements an ``init_app()`` method.
...
.. function:: object_list(template_name, query, context_variable='object_list', paginate_by=20, page_var='page', check_bounds=True, **kwargs)
    ...
    :param query: A :class:`SelectQuery` instance to paginate.
...
.. class:: PaginatedQuery(query_or_model, paginate_by, page_var='page', check_bounds=False)
    ...
    .. method:: get_object_list()
        ...
        Using the value of :meth:`~PaginatedQuery.get_page`, return the page
        of objects requested by the user. The return value is a
        :class:`SelectQuery` with the appropriate ``LIMIT`` and ``OFFSET``
        clauses.
