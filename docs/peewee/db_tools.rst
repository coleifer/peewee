...
applies pending ones in order.

Migrations can be auto-generated by diffing your application's models against
the database, see :ref:`generating-migration-diff`.
...
.. code-block:: console
   ...
   # If we include `-m` and give a path to our application models,
   # peewee can detect what changes are needed and pre-populate the
   # migration script.
   $ pwmigrate sqlite:///app.db -m my_app.models create "add karma"
   migrations/0002_add_karma.py
