...
In FastAPI you can create a dependency with ``yield`` to connect to the database.

Then you can use that dependency in the *path operations* that need to use the database.
...
.. code-block:: python
    ...
    # check_same_thread=False tells SQLite to allow connections from
    # different threads, as the functions will be run in threadpool
    ...
    # This dependency will connect to the database right before
    # handling a request
    def get_db():
        db.connect()
        try:
            ...
            # And here it will make sure the connection is closed
            # right after sending the response
            if not db.is_closed():
                db.close()


    # By adding Depends(get_db) to dependencies, FastAPI will know
    # it has to run that dependency for this path operation
    @app.get("/people/", dependencies=[Depends(get_db)])
    def read_people():
        return Person.select().get()



Notice that as Peewee doesn't use ``async`` and ``await``, you should declare your
FastAPI *path operation functions* and the dependency function using normal ``def``,
not ``async def``.

FastAPI will still run concurrently, but it will run those functions in a threadpool.
...
To support concurrent safety in async frameworks, Peewee uses Python `contextvars
<https://docs.python.org/3/library/contextvars.html>`_, available in Python 3.7+.

So, to use Peewee with FastAPI (or any other async framework) you need to use
Python 3.7 or above.

.. note::
    When using a version of Python below 3.7, Peewee uses `threading.local
    <https://docs.python.org/3/library/threading.html#thread-local-data>`_ to support
    thread safety, but that doesn't help when using async frameworks.
