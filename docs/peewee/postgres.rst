...
psycopg2 versus psycopg3
------------------------

Peewee supports both drivers and uses psycopg2 when both are installed,
unless ``prefer_psycopg3`` is given. Queries generated by peewee behave
identically on either. The difference is in how hand-written SQL is bound:
psycopg2 interpolates parameters into the query string on the client,
psycopg3 sends the query and parameters separately and Postgres binds them.
Server-side binding only accepts parameters in value positions of a single
plannable statement:
...
.. code-block:: python
   ...
   db = PostgresqlExtDatabase('my_app', prefer_psycopg3=True,
