...
.. class:: PostgresqlExtDatabase(database, server_side_cursors=False, register_hstore=False, prefer_psycopg3=True, **kwargs)
...
.. class:: Psycopg3Database(database, **kwargs)
   ...
   Same as :class:`PostgresqlExtDatabase`, which now prefers psycopg3 by
   default. Retained for compatibility.

.. class:: PooledPsycopg3Database(database, **kwargs)

   Connection-pooling variant of :class:`Psycopg3Database`.

.. _psycopg2-vs-psycopg3:

psycopg2 vs psycopg3
--------------------

Peewee supports both drivers, and will use whichever is installed. If **both**
psycopg2 and psycopg(3) are installed, Peewee defaults to psycopg3 unless
``prefer_psycopg3=False`` is specified:

.. code-block:: python

   # If both psycopg2 and psycopg3 are installed, use psycopg2.
   db = PostgresqlDatabase('peewee_test', prefer_psycopg3=False)

Queries generated by peewee behave identically on either. The difference is in
how hand-written SQL is bound: psycopg2 interpolates parameters into the query
string on the client, while psycopg3 sends the query and parameters separately
and Postgres binds them. psycopg3's server-side binding only accepts parameters
in value positions within a single query:

.. code-block:: python

   # Fails on psycopg3: SET cannot take a bound parameter.
   db.execute_sql('SET statement_timeout = %s', ('5s',))

   # Fails on psycopg3: multiple statements cannot be bound as one.
   db.execute_sql('UPDATE a SET x = %s; UPDATE b SET y = %s', (1, 2))

   # Fails on psycopg3: DDL cannot take bound parameters.
   db.execute_sql('ALTER TABLE t ADD COLUMN c INT DEFAULT %s', (0,))

Inline the values in such statements, or restore the psycopg2 behavior by
passing psycopg's ``ClientCursor``:

.. code-block:: python

   import psycopg

   db = PostgresqlDatabase('my_app', cursor_factory=psycopg.ClientCursor)

In exchange, psycopg3 automatically promotes frequently-executed queries to
server-side prepared statements, skipping repeated parse and plan overhead.
...
For large result sets, server-side (or named) cursors stream rows from the
server rather than loading the entire result into memory. Rows are fetched
transparently from the server as you iterate. A regular cursor, by contrast,
buffers the complete result set on the client at execute time.
...
To use server-side cursors, you must be using :class:`PostgresqlExtDatabase`.
...
.. code-block:: python
   ...
   # In a transaction the cursor streams rows on demand.
   with db.atomic():
...
Rows are fetched from the server in batches of ``array_size``, which defaults
to the driver's ``itersize`` (2000 with psycopg2, 100 with psycopg3). For
large scans specify ``array_size`` explicitly:

.. code-block:: python

   for page_view in ServerSide(large_query, array_size=1000):
       # Do something interesting.
       pass

Ordinarily, a server-side cursor is bound to a transaction and is destroyed when the
transaction ends. To use a server-side cursor outside a transaction, it must be
declared ``WITH HOLD``, which has a couple consequences:
...
Peewee declares ``WITH HOLD`` only when it has to:

* psycopg3: server-side cursors opened inside a transaction do not specify
  ``WITH HOLD``. The cursor streams normally and the cursor ends with the
  transaction. Iterating after the transaction closes will raise ``InvalidCursorName``.
* psycopg2 does not allow server-side cursors on autocommit connections, so
  peewee always declares ``WITH HOLD`` on psycopg2. Inside a transaction such a
  cursor streams normally, but if it is still open when the transaction
  commits, the remaining rows are copied and the cursor and resources survive.

Use server-side cursors inside a transaction whenever possible. When the result
fits in client memory, a regular (non-server-side) cursor is preferable.
...
.. warning::
   A cursor must be fully exhausted or explicitly closed to release its
   server resources. Holdable cursors (psycopg2 always, psycopg3 outside a
   transaction) otherwise persist until the connection closes.
