...
class TestLSMExtension(BaseTestCase):
    def setUp(self):
        ...
        if os.path.exists(KeyValue._meta.filename):
            os.unlink(KeyValue._meta.filename)

        database.connect()
        database.load_extension(LSM_EXTENSION.rstrip('.so'))
        database.create_tables([KeyValue])

    def tearDown(self):
        super(TestLSMExtension, self).tearDown()
        database.unload_extension(LSM_EXTENSION.rstrip('.so'))
        database.close()
        if os.path.exists(KeyValue._meta.filename):
            os.unlink(KeyValue._meta.filename)

    def test_lsm_extension_schema(self):
        self.assertSQL(KeyValue._schema._create_table(), (
            'CREATE VIRTUAL TABLE IF NOT EXISTS "keyvalue" USING lsm1 '
            '("test_lsm.ldb")'), [])

    def assertData(self, expected, query=None):
        if query is None:
            query = KeyValue.select().tuples()
        self.assertEqual(list(query), expected)

    def test_lsm_operations(self):
        # Create.
        with database.transaction():
            for i in range(3):
                KeyValue.create(key='k%s' % i, value='v%s' % i)

        # Read.
        self.assertData([('k0', 'v0'), ('k1', 'v1'), ('k2', 'v2')])

        # Delete (accomplished by inserting a value of NULL).
        insert = KeyValue.insert(key='k1', value=None)
        self.assertSQL(insert, (
            'INSERT INTO "keyvalue" ("key", "value") VALUES (?, ?)'),
            ['k1', None])

        insert.execute()
        self.assertData([('k0', 'v0'), ('k2', 'v2')])

        # Update.
        KeyValue.create(key='k0', value='v0-e')
        self.assertData([('k0', 'v0-e'), ('k2', 'v2')])

    def test_insert_many(self):
        data = [{'key': 'k%s' % i, 'value': 'v%s' % i} for i in range(4)]
        KeyValue.insert_many(data).execute()
        self.assertData([('k0', 'v0'), ('k1', 'v1'), ('k2', 'v2'),
                         ('k3', 'v3')])

    def test_delete_unsupported(self):
        with self.assertRaisesCtx(NotImplementedError):
            KeyValue.delete().where(KeyValue.key == 'k1').execute()

        KeyValue.insert_many({'key': 'k%s' % i, 'value': 'v%s' % i}
                             for i in range(3)).execute()
        self.assertData([('k0', 'v0'), ('k1', 'v1'), ('k2', 'v2')])

        with self.assertRaisesCtx(OperationalError):
            database.execute_sql('DELETE FROM "keyvalue"')

    def test_dict_methods(self):
        KeyValue['k1'] = 'v1'
        KeyValue['k2'] = 'v2'
        self.assertEqual(KeyValue['k1'], 'v1')
        self.assertEqual(KeyValue['k2'], 'v2')

        KeyValue['k1'] = 'v1-e'
        KeyValue['k3'] = 'v3'
        del KeyValue['k2']

        self.assertEqual(KeyValue['k1'], 'v1-e')
        self.assertRaises(KeyError, lambda: KeyValue['k2'])
        self.assertEqual(KeyValue['k3'], 'v3')
