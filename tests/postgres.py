...
class TestIdentityField(ModelTestCase):
    ...
    def test_schema(self):
        ...
        self.assertEqual(sql, (
            'CREATE TABLE "id_always" ("id" INTEGER NOT NULL PRIMARY KEY '
            'GENERATED ALWAYS AS IDENTITY, "data" VARCHAR(255) NOT NULL)'))

        sql, params = IDByDefault._schema._create_table(False).query()
        self.assertEqual(sql, (
            'CREATE TABLE "id_by_default" ("id" INTEGER NOT NULL PRIMARY KEY '
            'GENERATED BY DEFAULT AS IDENTITY, "data" VARCHAR(255) NOT NULL)'))
