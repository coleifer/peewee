...
class Func(Node):
    ...
    def __init__(self, name, *arguments):
        ...
        self._coerce = (
        name.lower() not in self._no_coerce) if name else False
...
class Window(Node):
    ...
    def __init__(self, partition_by=None, order_by=None, start=None,
                 end=None):
...
class Join(namedtuple('_Join', ('src', 'dest', 'join_type', 'on'))):
    ...
    def _join_metadata(self):
        ...
        on_field = isinstance(self.on,
                              (Field, FieldProxy)) and self.on or None
        if on_field:
            fk_field = on_field
...
class _AutoPrimaryKeyField(PrimaryKeyField):
    ...
    def __init__(self, *args, **kwargs):
        if 'undeclared' in kwargs and not kwargs['undeclared']:
            raise ValueError(
                '%r must be created with undeclared=True.' % self)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.resolution not in self.valid_resolutions:
            raise ValueError('TimestampField resolution must be one of: %s' %
                             ', '.join(
                                 str(i) for i in self.valid_resolutions))
        ...
        for src_model, joined_models in query._joins.items():
            ...
            for join_obj in joined_models:
                if isinstance(join_obj.dest, Node):
                    ...
                    new_map.add(join_obj.dest,
                                join_obj.dest._meta.table_alias)
...
class AggregateQueryResultWrapper(ModelQueryResultWrapper):
    ...
    def initialize(self, description):
        ...
        key_to_columns = {}
        for idx, (key, model_class, col_name, _) in enumerate(
                self.column_map):
    ...
    def iterate(self):
        ...
        while True:
            ...
            new_instances = self.construct_instances(cur_row,
                                                     different_models)
            ...
            if not self._is_multi_row_insert:
                if self.database.insert_returning:
                    ...
                    clean_data = [
                        ...
                        for field, column
                        in zip(meta.get_primary_key_fields(),
                               pk_row.values() if isinstance(pk_row,
                                                             dict) else pk_row)]
...
class SqliteDatabase(Database):
    ...
    def get_columns(self, table, schema=None):
        ...
        return [
            ColumnMetadata(row[1], row[2], not row[3], bool(row[5]), table)
            for row in cursor.fetchall()]
...
class PostgresqlDatabase(Database):
    ...
    def get_indexes(self, table, schema='public'):
        ...
        return [
            IndexMetadata(row[0], row[1], row[3].split(','), row[2], table)
            for row in cursor.fetchall()]
...
class ModelOptions(object):
    ...
    def rel_for_model(self, model, field_obj=None, multi=False):
        ...
        for field in self.sorted_fields:
            if isinstance(field,
                          ForeignKeyField) and field.rel_model == model:
                is_match = (
...
class BaseModel(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        ModelOptionsBase = meta_options.get('model_options_base',
                                            ModelOptions)
